...
class SNAKE:
    ...
    def move_snake(self):
        if self.new_block == True:
            body_copy = self.body[:] # Copia toda la lista
            body_copy.insert(0, body_copy[0] + self.direction)
            self.body = body_copy[:]
            self.new_block = False
        else:
            body_copy = self.body[:-1] # Copia toda la lista menos el último
            body_copy.insert(0, body_copy[0] + self.direction)
            self.body = body_copy[:]
...
# Crea un evento que se lanza cada x tiempo, ese tiempo se especifica en el timer
